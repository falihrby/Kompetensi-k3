<x-admin-layout>
    <div class="p-6">
        <div class="flex justify-between ml-2">
            <a href="{{ url()->previous() }}"
                class="px-4 py-2 text-white bg-green-600 rounded-md hover:bg-green-800">Kembali</a>
        </div>
                ...
                <nav class="text-sm" aria-label="Breadcrumb">
                    <ol class="inline-flex gap-4 p-0 list-none">
                        <li class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-gray-700">Data Kelulusan</span>
                        </li>
                    </ol>
                </nav>
            </div>
            <div class="flex items-center mt-4 space-x-2 sm:mt-0">
                <a href="{{ route('laporan.kelulusan.cetak', ['search' => request()->input('search')]) }}"
                    target="_blank"
        ...
        <div class="p-4 mb-10 overflow-hidden bg-white shadow-sm sm:rounded-lg">
            <div class="flex items-center justify-between p-2">
                <div class="text-base font-semibold text-gray-900">
                    {{ __('Data Kelulusan Peserta') }}
                </div>
                <div class="col-md-6">
                    <form action="{{ route('laporan.kelulusan.data-kelulusan') }}" method="GET">
                        ...
                        @foreach ($kelulusanData as $kelulusan)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    {{ str_pad($kelulusan->id, 4, '0', STR_PAD_LEFT) }}</td>
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    {{ $kelulusan->nama }}</td>
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    {{ $kelulusan->nomor }}</td>
                                ...
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    {{ $kelulusan->updated_at }}</td>
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    <a href="{{ route('laporan.kelulusan.show', $kelulusan->id) }}"
