<x-admin-layout>
                ...
                <nav class="text-sm" aria-label="Breadcrumb">
                    <ol class="inline-flex gap-4 p-0 list-none">
                        <li class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-gray-700">Program Studi</span>
                        </li>
                    </ol>
                </nav>
            </div>
                        ...
                        @foreach ($programStudis as $programStudi)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-xs align-top whitespace-nowrap">
                                    {{ str_pad($programStudi->id, 4, '0', STR_PAD_LEFT) }}</td>
                                ...
                                <td class="flex px-4 py-2 space-x-1 text-xs font-medium align-top whitespace-nowrap">
                                    <button data-id="{{ $programStudi->id }}" data-nama="{{ $programStudi->nama }}"
                                    ...
                                    <button type="button" data-id="{{ $programStudi->id }}"
                                        data-action="{{ route('program-studi.destroy', $programStudi->id) }}"
    ...
    <div id="createModal" class="fixed inset-0 z-50 items-center justify-center hidden">
        ...
        <div class="p-6 align-middle transition-all transform bg-white rounded-lg shadow-xl sm:max-w-lg sm:w-full">
            <div class="flex items-start justify-between">
                <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Tambah Program Studi</h3>
            ...
            <form action="{{ route('program-studi.store') }}" method="POST" class="mt-5">
                @csrf
                ...
                <div class="flex items-center my-4 space-x-4">
                    <label for="nama" class="w-1/3 text-sm font-medium text-gray-700">Nama Program
                        Studi<span>:</span></label>
    ...
    <div id="editModal" class="fixed inset-0 z-50 items-center justify-center hidden">
        ...
        <div class="p-6 align-middle transition-all transform bg-white rounded-lg shadow-xl sm:max-w-lg sm:w-full">
            <div class="flex items-start justify-between">
                <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Edit Program Studi</h3>
            ...
            <hr class="h-px m-4 bg-gray-100 border-0 dark:bg-gray-200">
            <form id="editForm" action="{{ route('program-studi.update', 0) }}" method="POST" class="mt-5">
                @csrf
                @method('PUT')
                <div class="flex items-center my-2 space-x-4">
                ...
                <div class="flex items-center my-4 space-x-4">
                    <label for="editNama" class="w-1/3 text-sm font-medium text-gray-700">Nama Program
                        Studi<span>:</span></label>
    ...
    <div id="delete-confirmation-modal"
        ...
        <div class="flex flex-col justify-center items-center px-5 py-6 bg-white rounded-2xl shadow-sm max-w-[342px]">
            ...
            <h3 class="mt-2 text-xl font-bold text-center text-slate-800">Hapus Program Studi</h3>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    editForm.action = `{{ url('program-studi') }}/${id}`;
